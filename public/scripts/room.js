...
room.getRoom=function(f){if(bradev.device==="iphone"){var d="iphone"}else{var d="other"}if(room.now===undefined){room.now=""}$.ajax({url:"/getRoom?"+f+room.now+"_"+d+"?"+room.getLang(),cache:false,success:function(data){room.$room.html(data);$('.screen.loader').fadeOut()}})};
...
room.setContent=function(){if(room.getLang()!=="ja"){room.$create.text("Create Room");room.$manage.text("Manage Room");room.$what.text("What's Room?")}else{room.$body.addClass("ja");room.$create.text("部屋の作成");room.$manage.text("部屋の管理");room.$what.text("部屋について")}};
...
room.init();
room.close=function(){if(room.mode==="view"){if(bradev.browser!=="safari"&&bradev.device==="other"){room.$body.removeClass("view2")}else{room.$body.removeClass("view").removeClass("view2").removeClass("mobile")}setTimeout(function(){room.$body.removeClass("view");room.mode="main"},280)}else if(room.mode==="layout"){$('.screen.layer.layout .loader').fadeOut();room.$body.removeClass("layout2");setTimeout(function(){room.refreshExplain();room.$body.removeClass("layout");room.mode="view"},280)}};
room.back = function() {};
room.view=function(){room.mode="view";if(bradev.browser!=="safari"&&bradev.device==="other"){room.refreshExplain();room.$body.addClass("view")}else{room.$body.addClass("view").addClass("mobile")}setTimeout(function(){room.$body.addClass("view2")},280)};
